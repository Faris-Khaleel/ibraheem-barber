...
- Mobile is a first-class presentation: keep the transformation subject large enough to read, keep the muted inline video actively paintable during coarse-pointer scrolling, and advance the real four-stage fallback artwork with scroll progress so iPhone Safari can never remain on the empty chair after the chapter changes. Keep the chapter rail clear of the floating booking control, and hide the floating WhatsApp control from the Contact section onward so it never obscures the phone actions.
